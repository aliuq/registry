...
type KnownTheme
  = | 'auto/asciinema'
    ...
    | 'auto/nord'
    | 'auto/solarized-dark'
    | 'auto/solarized-light'
    | 'auto/tango'

type AnyTheme = string & {}

type Theme = KnownTheme | AnyTheme
type Fit = 'width' | 'height' | 'both' | false | 'none'
...
export interface Props {
   ...
   * synced with the session playback - pausing/resuming/seeking is reflected in the audio playback.
   *
   * For live terminal streams audioUrl is expected to be a live audio source - either a directHTTP
   * audio stream (.mp3, .aac, .ogg, etc) such as Icecast/Shoutcast endpoint, or HLS playlist (.m3u8).
   */
  audioUrl?: string
  /**
...
onMounted(async () => {
  if (!window || !document || !playerRef.value)
    return

  // @ts-expect-error ignore
  const AsciinemaPlayer = await import('asciinema-player').then(mod => mod.default || mod)
  asciinemaPlayer.value = AsciinemaPlayer.create(props.src, playerRef.value, {
    logger: console,
    ...{
      cols: isDesktop.value ? (props.cols || 120) : 90,
      rows: isDesktop.value ? (props.rows || 24) : 20,
    },
    ...cleanObject(props),
  })
})
...
onUnmounted(() => {
  if (!window || !document || !asciinemaPlayer.value)
    return
...
<template>
  <template v-if="props.src">
    <div ref="playerRef" class="max-w-full sm:max-w-160 lg:max-w-3xl xl:max-w-200 overflow-hidden" />
  </template>
</template>
